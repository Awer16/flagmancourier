...
export default function EnterpriseLoginForm(): React.ReactElement {
  ...
  return (
    <form className="mt-8 flex flex-col gap-5" onSubmit={onSubmit} noValidate>
      ...
      <div className="flex flex-col gap-1.5">
        <label htmlFor="enterprise-login-email" className={labelClass}>
          Электронная почта
        </label>
        <input
      ...
      <div className="flex flex-col gap-1.5">
        <label htmlFor="enterprise-login-password" className={labelClass}>
          Пароль
        </label>
        <div className="relative">
          ...
          >
            <i className={`fas ${passwordVisible ? "fa-eye-slash" : "fa-eye"} text-sm`} aria-hidden />
          </button>
        </div>
      </div>
      <button
      ...
      >
        {loading ? (
          <><i className="fas fa-spinner fa-spin text-sm" aria-hidden /> Вход...</>
        ) : (
          <><i className="fas fa-right-to-bracket text-sm" aria-hidden /> Войти как предприятие</>
        )}
      </button>
      <p className="text-center text-sm text-muted">
        Нет аккаунта?{" "}
        <Link href="/enterprise/register" className="font-medium text-primary transition-colors hover:text-primary-hover">
          Зарегистрироваться
        </Link>
      </p>
    </form>
